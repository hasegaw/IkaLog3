...
class Spl3GameRankedBattleEvents(StatefulScene):

    # Called per Engine's reset.
    def reset(self):
        super(Spl3GameRankedBattleEvents, self).reset()
        ...
        self.enable_all_masks()  # debug

    def enable_all_masks(self):
        # for debug.
        self._masks_active.update(self._masks_splatzone)
        self._masks_active.update(self._masks_rainmaker)
        self._masks_active.update(self._masks_towercontrol)
        self._masks_active.update(self._masks_cramblitz)

        # Initialize Multi-Class IkaMatcher
        self._masks_active2 = MultiClassIkaMatcher()
        for mask in self._masks_active.keys():
    ...
    def _state_triggered(self, context):
        ...
        if most_possible != self._last_mask_matched:
            logger.info(f"matched {most_possible}")
    ...
    def _init_scene(self, debug=False):
        ...
        self.mask_they_lead = IkaMatcher2(
            448, 184, 384, 48,
...
if __name__ == "__main__":
    Spl3GameRankedBattleEvents.main_func()
