...
class Spl3GameDead(StatefulScene):
    ...
    def recoginize_and_vote_death_reason(self, context):
        ...
        # FIXME: Need condition update for Splatoon 3
        # New Condition: background is dark, colored (was: gray)
        img_weapon_color = np.average(img_weapon_hsv[:, :, 1].reshape(-1)) 
        img_weapon_colored = img_weapon_color > 130
    ...
    def _state_default(self, context):
        ...
        session = self.find_scene_object('Spl3GameSession')
    ...
    def _state_tracking(self, context):
        ...
        # 画面が続いているならそのまま
        if matched:
            # if not self.is_another_scene_matched(context, 'Spl3InGame'):
            return True
    ...
    def _init_scene(self, debug=False, train=True, preview=True):
        self.train = train
